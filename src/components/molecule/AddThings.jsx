...
export const AddThings = ({
  ...
  const getOptions = (objectProperty, selectedTab) => {
    let sectionOptions = [];
    const selectedObject = mockObjects.find(
  ...
  const handleAddClick = () => {
    setBoolOptions((prevState) => {
      const foundIndex = prevState.findIndex(
        (object) => object.category === categoryInputValue,
      );
      ...
      if (foundIndex < 0) {
        ...
        return [
          ...prevState,
          { category: categoryInputValue, options: [optionInputValue] },
        ];
      } else {
    ...
    <div className="mb-4">
      ...
      {isAdding && (
        <div className="flex max-h-[1.8rem]">
          ...
          <InputField
            getInputValue={getInputFieldValue}
            inputPlaceHolder={optionPlaceHolder}
          />
